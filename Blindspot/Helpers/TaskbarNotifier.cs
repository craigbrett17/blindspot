...
// Additions by Craig Brett for use in Blindspot
// 31 May 2014 Made this able to work without a Bitmap because that was hamstringing me a bit
//          There is now a method that uses a Bitmap's width and height if present for show/hide animations, 
//                  but falls back to using the Rectangle width and height if not
//          Added 2 new public fields for whether changing the title or text on a shown
//                  notification resets the timer for how long the message is shown
//          Added public fields to set the alignment of title and content text
//          Added a public field to set how far from the right edge of the screen the notification is
...
namespace Blindspot.Helpers
{
	...
	public class TaskbarNotifier : System.Windows.Forms.Form
	{
        ...
        public bool ChangeTitleResetsTimer = false;
        public bool ChangeContentResetsTimer = false;
        public StringAlignment TitleAlignment = StringAlignment.Near;
        public StringAlignment ContentAlignment = StringAlignment.Center;
        public int PixelsFromRightEdgeOfScreen = 17;
		#endregion
		...
		public string TitleText
		{
			...
			set
			{
				titleText=value;
				Refresh();
                if (taskbarState == TaskbarStates.visible && ChangeTitleResetsTimer)
                    ResetTimer();
			}
		}
		...
		public string ContentText
		{
			...
			set
			{
				contentText=value;
				Refresh();
                if (taskbarState == TaskbarStates.visible && ChangeContentResetsTimer)
                    ResetTimer();
			}
		}
        ...
        /// <summary>
        /// Gets the height of the shown area, either by the background bitmap or from the totals of the drawn areas
        /// </summary>
        public int BitmapOrRectanglesHeight
        {
            get
            {
                return (BackgroundBitmap != null)
                    ? BackgroundBitmap.Height
                    : ContentRectangle.Height + TitleRectangle.Height;
            }
        }

        /// <summary>
        /// Gets the width of the shown area, either by the width of the Background bitmap or the largest rectangle out of title or content
        /// </summary>
        public int BitmapOrRectanglesWidth
        {
            get
            {
                return (BackgroundBitmap != null)
                    ? BackgroundBitmap.Width
                    : Math.Max(ContentRectangle.Width, TitleRectangle.Width);
            }
        }
		...
		public void Show(string strTitle, string strContent, int nTimeToShow, int nTimeToStay, int nTimeToHide)
		{
			...
			if (nTimeToShow > 10)
			{
				nEvents = Math.Min((nTimeToShow / 10),  BitmapOrRectanglesHeight);
				nShowEvents = nTimeToShow / nEvents;
				nIncrementShow = BitmapOrRectanglesHeight / nEvents;
			}
			else
			{
				nShowEvents = 10;
				nIncrementShow = BitmapOrRectanglesHeight;
			}

			// We calculate the pixel increment and the timer value for the hiding animation
			if( nTimeToHide > 10)
			{
				nEvents = Math.Min((nTimeToHide / 10), BitmapOrRectanglesHeight);
				nHideEvents = nTimeToHide / nEvents;
				nIncrementHide = BitmapOrRectanglesHeight / nEvents;
			}
			else
			{
				nHideEvents = 10;
				nIncrementHide = BitmapOrRectanglesHeight;
			}
			...
			switch (taskbarState)
			{
				case TaskbarStates.hidden:
					...
					SetBounds(WorkAreaRectangle.Right-BitmapOrRectanglesWidth-PixelsFromRightEdgeOfScreen, WorkAreaRectangle.Bottom-1, BitmapOrRectanglesWidth, 0);
				...
				case TaskbarStates.disappearing:
					...
					SetBounds(WorkAreaRectangle.Right-BitmapOrRectanglesWidth-PixelsFromRightEdgeOfScreen, WorkAreaRectangle.Bottom-BitmapOrRectanglesHeight-1, BitmapOrRectanglesWidth, BitmapOrRectanglesHeight);
					timer.Interval = nVisibleEvents;
					timer.Start();
					Refresh();
					break;
		...
		protected void OnTimer(Object obj, EventArgs ea)
		{
			...
			{
				case TaskbarStates.appearing:
					if (Height < BitmapOrRectanglesHeight)
						SetBounds(Left, Top-nIncrementShow ,Width, Height + nIncrementShow);
					else
					{
						timer.Stop();
						Height = BitmapOrRectanglesHeight;
						timer.Interval = nVisibleEvents;
						taskbarState = TaskbarStates.visible;
						timer.Start();
					}
		...
		protected override void OnPaintBackground(PaintEventArgs pea)
		{
			...
			offscreenBitmap = new Bitmap(BitmapOrRectanglesWidth, BitmapOrRectanglesHeight);
            ...
            {
                offScreenGraphics.FillRectangle(new SolidBrush(BackColor), 0, 0, BitmapOrRectanglesWidth, BitmapOrRectanglesHeight);
            }
